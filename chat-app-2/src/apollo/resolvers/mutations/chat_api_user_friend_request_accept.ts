...
export const chat_api_user_friend_request_accept = async (
  root: any,
  args: any,
  ctx: any
): Promise<any> => {
  ...
  try {
    saveRequestLog(ticket, args, chat_api_user_friend_request_accept.name, clientIp)
    ...
    await session.withTransaction(async () => {
      ...
      saveSuccessLog(ticket, args, chat_api_user_friend_request_accept.name, finalResult.message, clientIp)
    }, transactionOptions)
    return finalResult
  } catch (e) {
    ...
    saveErrorLog(ticket, args, chat_api_user_friend_request_accept.name, errorResult, clientIp)
    if (session.inTransaction()) {
      await session.abortTransaction();
    }
    ErrorResolve(e, args, chat_api_user_friend_request_accept.name)
  } finally {
    session.endSession()
  }
}
