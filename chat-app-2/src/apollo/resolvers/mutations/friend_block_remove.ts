...
const friend_block_remove = async (root: any, args: any, ctx: any): Promise<any> => {
    ...
    try {
       ...
       let reciverSlug = await getSlugByToken(token)
        ...
        if (findUsersRes.length !== 2) throw new Error("someone not exist in database!");
        //Start transaction
        ...
        const updateDoc = checkFriend._blockRequest.length === 1 ? { $set: { isBlock: false }, $pull: { _blockRequest: reciverSlug } } : { $set: { isBlock: true }, $pull: { _blockRequest: reciverSlug } }
