...
const chat_get_mix_rooms = async (root: any, args: any, ctx: any) => {
    ...
    try {
        ...
        const allRooms:InboxRoom[] = await db.collection(collectionNames.rooms).find({ $or: [{ type: RoomTypes.public }, { type: RoomTypes.global },{ pair: { $all: [{ slug }] } }] }).sort({"lastMess.sentAt":-1}).limit(limit).skip(skip).toArray()
         const sortFunc = (a, b) => {
