...
const chat_app_public_room_add_member = async (
    root: any,
    args: any,
    ctx: any
): Promise<any> => {

    console.log("======PUBLIC ROOM ADD MEMBER=====")
    //Get arguments
    console.log({ args });
    ...
    try {
        ...
        if (foundChatRoom) {
            ...
            if (foundChatRoom.createdBy.slug === master) {
                ...
                if (checkIsOldMemberRes.length === 0) {
                    let allAddMemberQuery = { slug: { $in: member } }
                    let updateAllAddMemberData = await db.collection(collectionNames.users).updateMany(allAddMemberQuery, { $push: { chatRooms: objectChatRoomId } })
                ...
                session.endSession()
                throw new Error(`someone is already a member of this room`)
            }
            await session.abortTransaction()
            session.endSession()
...
export { chat_app_public_room_add_member };
